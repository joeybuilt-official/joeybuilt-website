...
export function ProductCard({ product }: { product: Product }) {
  const Name = product.hasLiveSite ? (
    <a
      href={product.url}
      target="_blank"
      rel="noopener noreferrer"
      className="font-[family-name:var(--font-mono)] text-xl font-semibold text-base-100 transition-colors hover:text-accent-400"
    >
      {product.name} &rarr;
    </a>
  ) : (
    <span className="font-[family-name:var(--font-mono)] text-xl font-semibold text-base-100">
      {product.name}
    </span>
  );
